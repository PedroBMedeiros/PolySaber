...
// Saber thresholds - adjust these after testing
const float STRIKE_THRESHOLD = 19.0; // Acceleration in m/s^2
const float SWING_THRESHOLD  = 1.5;  // Rotation in rad/s
...
void loop2() { //for tuning
  ...
  // Print values for Plotter (labels make it easier to see)
  Serial.print("Accel:"); Serial.print(accelMag);
  Serial.print(",");
  Serial.print("Strike_Limit:"); Serial.print(STRIKE_THRESHOLD);
  Serial.print(",");
  Serial.print("Gyro:"); Serial.print(gyroMag * 5); // Multiplied by 5 to see it better on the same scale
  Serial.print(",");
  Serial.print("Swing_Limit:"); Serial.println(SWING_THRESHOLD * 5);

  delay(10); // Slow down plot just for tuning
}
...
//Clean switching loop

//if (gyroMag > SWING_THRESHOLD) {
//    // If we were humming, the swing takes over immediately
//    update_swing_loop(4); // Medium volume swing
//    update_led_breathing(true); // "Moving" LED state
//} 
//else {
//    // Return to idle
//    update_hum(6); // Quiet idle hum
//    update_led_breathing(false); // "Calm" LED state
//}
